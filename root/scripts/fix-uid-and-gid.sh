...
DOCKER_GROUP_CURRENT_ID=`id -g $USER`

if [ $DOCKER_GROUP_CURRENT_ID -eq $USER_GID ]; then
    echo "Group $USER is already mapped to $DOCKER_GROUP_CURRENT_ID. Nice!"
else
    ...
    echo "Changing the ID of $USER group to $USER_GID"
    groupmod -o -g $USER_GID $USER || true
    echo "Finished"
    echo "-- -- -- -- --"
fi
...
DOCKER_USER_CURRENT_ID=`id -u $USER`

if [ $DOCKER_USER_CURRENT_ID -eq $USER_ID ]; then
    echo "User $USER is already mapped to $DOCKER_USER_CURRENT_ID. Nice!"
    ...
    echo "Changing the ID of $USER user to $USER_ID"
    usermod -o -u $USER_ID $USER || true
fi